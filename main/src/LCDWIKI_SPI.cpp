...
LCDWIKI_SPI::LCDWIKI_SPI(uint16_t model,int8_t cs, int8_t cd, int8_t miso, int8_t mosi, int8_t reset, int8_t clk, int8_t led)
{
	...
	spicsPort = portOutputRegister(digitalPinToPort(cs)); // Address of the register that control CS port
...
LCDWIKI_SPI::LCDWIKI_SPI(uint16_t model,int8_t cs, int8_t cd, int8_t reset, int8_t led)
{
	...
	// Init MISO, MOSI, SPI_CLK for hardware SPI
	spimisoPort = 0;
	spimisoPinSet = 0;
	spimisoPinUnset = 0;
	spimosiPort = 0;
...
{
	CS_ACTIVE;
	writeCmdData16(cmd,data); //CD_COMMAND; write8(a>>8); write8(a); CD_DATA; write8(d>>8); write8(d)
	CS_IDLE;
}
...
void LCDWIKI_SPI:: init_table16(const void *table, int16_t size)
{
    ...
    while (size > 0) 
	{
		...
		{
			//CS_ACTIVE: CD_COMMAND; write8(cmd>>8); write8(cmd); CD_DATA; write8(d>>8); write8(d); CS_IDLE;
			//write8(cmd) = Spi_Write(uint8_t data)
